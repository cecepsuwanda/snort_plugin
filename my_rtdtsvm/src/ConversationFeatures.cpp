...
void ConversationFeatures::print(bool print_extra_features) const
{
    ...
    if (print_extra_features) {
		...
		strftime(timestr, sizeof timestr, "%m/%d/%Y,%H:%M:%S", ltime);
		...
		ltime = localtime(&local_tv_sec);
		//localtime_s(&timeinfo, &local_tv_sec);
		strftime(timestr1, sizeof timestr1, "%m/%d/%Y,%H:%M:%S", ltime);
		//strftime(timestr, sizeof timestr, "%Y-%m-%dT%H:%M:%S", &timeinfo);
		ss << timestr1;
	}
	ss << ',';
...
vector<string> ConversationFeatures::get_attr()
{
	vector<string> tmp;
...
void ConversationFeatures::set_label(string vlabel)
{
	label = vlabel;
}
