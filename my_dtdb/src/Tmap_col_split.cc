...
void Tmap_col_split::cek_valid_attr(int jml_data_cabang,int jml_data_root)
{
	...
	string tmp = "select * from attr_stat where (jml>=2) and (is_continue=1) and ( jml <= round(" + to_string(global_config.ratio_valid_attr) + "*" + to_string(jml_data_cabang) + ",2))  order by id"; // where (is_continue=0) or ((is_continue=1) and  (jml <= round("+to_string(global_config.ratio_valid_attr)+"*" + to_string(jml_row) + ",2)) and  (jml>=2))
